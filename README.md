...
반복적 DNS 해석으로 영화 매니페스트를 받아오고, 클라이언트가 버퍼 상태를 평활화한 값(R_buffer)을 기준으로 화질을 동적으로 전환한다. 공격자가 가짜 응답을 burst로 쏘아 캐시를 오염시키려 할 때, txid 검증·SPR·0x20 인코딩 세 계층의 누적 엔트로피로 막아낸다.
